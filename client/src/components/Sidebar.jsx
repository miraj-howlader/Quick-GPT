...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  const {createNewChat,axios,setChats,fetchUserChats,token,setToken, chats, setSelectedChat,navigate, theme, setTheme, user } = useAppContext();
  ...
  return (
    <div
      className={`flex flex-col h-screen min-h-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 
    to-[000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-300
    ...
    >
      ...
      {/* Search Conversation  */}
      <div
        className="flex items-center gap-2 mt-4 p-3 border border-gray-400 dark:border-white/20
       rounded-md"
      >
    ...
    includes(search.toLowerCase())).map((chat)=>(
       ...
       onClick={()=> {navigate('/'); setSelectedChat(chat); setIsMenuOpen(false)}}
        key={chat._id} className="p-2 px-4 dark:bg-[#57317c]/10 border
                border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex
                 justify-between group">
                  ...
                  <img onClick={e=>toast.promise(deleteChat(e,chat._id),{loading:"deleting..."})} src={assets.bin_icon}className="hidden group-hover:block w-4 cursor-pointer
                   not-dark:invert" alt="" />
      ...
      <div
       ...
       className="flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15
       rounded-md ">
